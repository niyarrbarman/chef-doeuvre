...
# ============================================================================
# GENDER PREDICTION ONLY
# ============================================================================
# torchrun --nproc_per_node=<gpus> --nnodes=<nodes> \
#          --node_rank=0 --master_addr=<ip> --master_port=29500 gender_predict.py
# ============================================================================

# ========================= CONFIG =========================
CSV_PATH = "/tmpdir/m25211shrm/musiclm/final-merged-ds.csv"
OUTPUT_PATH = "/tmpdir/m25211shrm/musiclm/gender_predictions.csv"
...
# ====================== GENDER PROMPT ======================
GENDER_SYSTEM = """You are analyzing song lyrics to predict the artist's gender.

Based on the lyrics, predict:
- Artist Gender: Must be EXACTLY either "Male" or "Female"

Output ONLY valid JSON:
{"artist_gender": "Male", "reasoning": "Brief explanation"}

NO extra text before or after JSON."""

def build_prompt(lyrics: str):
    return [
        {"role": "system", "content": GENDER_SYSTEM},
        {"role": "user", "content": f"Predict artist gender from these lyrics:\n\n{lyrics[:1500]}"}
    ]
...
def normalize_gender(gender_str: str) -> str:
    ...
    if any(x in g for x in ["female", "woman", "girl", "she"]):
        return "Female"
    return "Male"
...
def parse_output(text: str) -> dict:
    ...
    if parsed:
        return {"artist_gender": normalize_gender(parsed.get("artist_gender", "Male"))}
    return {"artist_gender": "Male"}
...
def infer_batch(lyrics_list: list) -> list:
    ...
    if not valid_lyrics:
        return [{"artist_gender": "Male"}] * len(lyrics_list)
...
for batch_idx in tqdm(range(num_batches), desc=f"Rank {RANK}", disable=(RANK != 0)):
    ...
    for i, idx in enumerate(batch_indices):
        my_results.append({"idx": idx, "predicted_gender": results_parsed[i]["artist_gender"]})
...
if RANK == 0:
    for result in all_results:
        idx = result["idx"]
        df.at[idx, "predicted_gender"] = result["predicted_gender"]

    df.to_csv(OUTPUT_PATH, index=False)
    print(f"\nSaved to {OUTPUT_PATH}")

    # Calculate accuracy
    if 'gender' in df.columns: # <--- FIX APPLIED HERE
        df['gt_gender'] = df['gender'].apply(normalize_gender)
        correct = (df['predicted_gender'] == df['gt_gender']).sum()
        total = len(df)
        accuracy = correct / total
        print(f"\n{'='*50}")
        print(f"GENDER PREDICTION ACCURACY: {accuracy:.4f} ({correct}/{total})")
        print(f"{'='*50}")
        print(f"\nPredictions:\n{df['predicted_gender'].value_counts()}")
        print(f"\nGround Truth:\n{df['gt_gender'].value_counts()}")
    else:
        print("\nWARNING: Ground truth column 'gender' not found. Accuracy not calculated.")
